...
export async function POST(req: NextRequest) {
  ...
  // Upsert — same email can be re-invited (updates label/invitedAt)
  await db
    .insert(allowedEmails)
    .values({ email, label, invitedBy: SUPERADMIN })
    .onConflictDoUpdate({
      target: allowedEmails.email,
      set: { label, invitedBy: SUPERADMIN, invitedAt: new Date() },
    });
  ...
  try {
    ...
    await transport.sendMail({
      ...
      html: buildInviteEmail(email, label, APP_URL),
    });
    emailSent = true;
  } catch (e) {
...
function buildInviteEmail(email: string, label: string | null, appUrl: string): string {
  return `<!DOCTYPE html>
<html lang="es">
  ...
  <div style="max-width:540px;margin:32px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.08);">
    ...
    <div style="padding:32px 40px;">
      <p style="color:#374151;font-size:15px;margin:0 0 20px;">
        Te invitaron a acceder al <strong>Monitor de Concursos Docentes CGE Entre Ríos</strong>.
      </p>
