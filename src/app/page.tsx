import { getDb } from '@/db'
import { config as configTable, state as stateTable } from '@/db/schema'
import { getUser } from '@/lib/get-user'
import Dashboard from '@/components/Dashboard'

export const dynamic = 'force-dynamic'

const GOOGLE_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="20" height="20">
  <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
  <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
  <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l3.66-2.84z"/>
  <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
</svg>`

export default async function Home() {
  const auth = await getUser()

  if (!auth.ok) {
    const loginUrl = `https://accounts.puntoindigo.com/api/auth/signin-google?next=${encodeURIComponent(process.env.APP_URL ?? 'https://concursos-cge.puntoindigo.com')}`

    if (auth.reason === 'not_allowed') {
      return (
        <div style={{ background: 'linear-gradient(135deg,#4338ca 0%,#6366f1 100%)', minHeight: '100vh' }}
          className="flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
            <div className="px-8 py-10 text-center">
              <div className="text-4xl mb-4">🔒</div>
              <h2 className="text-xl font-bold text-gray-900 mb-2">Acceso no autorizado</h2>
              <p className="text-gray-500 text-sm mb-6">
                Tu cuenta de Google no tiene acceso a esta aplicación. Contactá al administrador para solicitar acceso.
              </p>
              <a href="https://accounts.puntoindigo.com/api/auth/logout"
                className="text-indigo-600 text-sm font-medium hover:underline">
                Cerrar sesión
              </a>
            </div>
            <div className="bg-gray-50 border-t border-gray-100 py-3 px-8 text-center">
              <p className="text-xs text-gray-400">
                Desarrollado por{' '}
                <a href="https://puntoindigo.com" className="text-indigo-600 font-medium hover:underline">Puntoindigo</a>
              </p>
            </div>
          </div>
        </div>
      )
    }

    // no_cookie or invalid_token → show login page
    return (
      <div style={{ background: 'linear-gradient(135deg,#4338ca 0%,#6366f1 100%)', minHeight: '100vh' }}
        className="flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden">
          <div className="px-8 py-10 text-center">
            <p className="text-indigo-600 text-xs font-bold uppercase tracking-widest mb-3">
              Consejo General de Educación · Entre Ríos
            </p>
            <h1 className="text-2xl font-bold text-gray-900 mb-2 leading-tight">
              Monitor de Concursos Docentes
            </h1>
            <p className="text-gray-500 text-sm mb-8 leading-relaxed">
              Seguí las convocatorias a concurso del CGE con filtros por departamento, nivel y alertas automáticas por email.
            </p>
            <a
              href={loginUrl}
              className="inline-flex items-center gap-3 border-2 border-gray-200 hover:border-indigo-400 rounded-xl px-6 py-3 text-sm font-semibold text-gray-700 hover:text-indigo-700 transition-all shadow-sm hover:shadow-md bg-white"
            >
              <span dangerouslySetInnerHTML={{ __html: GOOGLE_SVG }} />
              Acceder con Google
            </a>
          </div>
          <div className="bg-gray-50 border-t border-gray-100 py-3 px-8 text-center">
            <p className="text-xs text-gray-400">
              Desarrollado por{' '}
              <a href="https://puntoindigo.com" target="_blank" rel="noopener noreferrer" className="text-indigo-600 font-medium hover:underline">
                Puntoindigo
              </a>
            </p>
          </div>
        </div>
      </div>
    )
  }

  const user = {
    email: auth.user.email,
    name: auth.user.name ?? '',
    picture: auth.user.picture ?? '',
    isSuperadmin: auth.user.email.toLowerCase() === 'daeiman@gmail.com',
  }

  let initialConfig = null
  let initialState = null

  try {
    const db = getDb()
    const [cfg] = await db.select().from(configTable).limit(1)
    const [st] = await db.select().from(stateTable).limit(1)
    initialConfig = cfg ?? null
    initialState = st
      ? {
          lastRunAt: st.lastRunAt?.toISOString() ?? null,
          lastFoundCount: st.lastFoundCount,
          lastPostDate: st.lastPostDate ?? null,
        }
      : null
  } catch {
    // DB not ready yet — show dashboard with defaults
  }

  return (
    <Dashboard
      user={user}
      initialConfig={
        initialConfig
          ? {
              categoryDepts: initialConfig.categoryDepts as number[],
              categoryLevels: initialConfig.categoryLevels as number[],
              searchString: initialConfig.searchString ?? '',
              emailTo: initialConfig.emailTo ?? '',
              scheduleCron: initialConfig.scheduleCron ?? '0 10 * * *',
              isActive: initialConfig.isActive ?? true,
            }
          : null
      }
      initialState={initialState}
    />
  )
}
